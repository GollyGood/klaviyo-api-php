...
class KlaviyoApiTest extends KlaviyoTestCase {
  ...
  public function testCanGetApiEndpoint() {
    $this->assertEquals($this->endPoint, KlaviyoApi::$endPoint, 'The endpoint was not set or retrievable as expected.');
  }
  ...
  public function testRequestWithEndPointAndApiKey() {
    ...
    $this->assertEquals('GET', $request->getMethod(), 'The request did not use GET.');
    $this->assertEquals($this->endPoint . '/api/v1?api_key=thisisakey', (string) $request->getUri(), 'The request created the wrong Uri.');
